...
export const DashboardPage: React.FC<DashboardPageProps> = ({ setCurrentTab }) => {
  ...
  // Scoped datasets strictly for active company
  const invoices = rawInvoices.filter(inv => isItemForActiveCompany(inv));
  const customers = rawCustomers.filter(c => isItemForActiveCompany(c));
  const products = rawProducts.filter(p => isItemForActiveCompany(p));
        ...
        >
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 dark:text-slate-400">
              {activeCompany?.name || 'Active Business'}
            </div>
          </div>
        </button>
